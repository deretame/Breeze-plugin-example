...
expectType<Promise<string>>(
  bridge.call("save_plugin_config", "auth.token", "abc"),
);
...
expectType<string | undefined>(
  maybeCrypto?.createHmac("sha256", "key").update("hello").digest("hex") as
    | string
    | undefined,
);
...
expectType<string>(
  crypto.createHash("sha256").update("hello").digest("hex") as string,
);
